// server.js - AuthAPI v3.2 with Anti-Crash, Data Persistence & Advanced Features
...
app.post('/api/extend-key', requireAuth, (req, res) => {
  try {
    ...
    logActivity('extend_key', req.user.userId, req.user.username, { 
      keyCode: key, days 
    });
...
app.post('/api/reset-key', requireAuth, (req, res) => {
  try {
    ...
    logActivity('reset_key', req.user.userId, req.user.username, { 
      keyCode: key, devicesCleared: oldDevices 
    });
...
app.post('/api/delete-key', requireAuth, (req, res) => {
  try {
    ...
    logActivity('delete_key', req.user.userId, req.user.username, { 
      keyCode: key 
    });
...
app.get('/api', (req, res) => {
  const config = loadConfig();
  res.json({
    name: "AuthAPI v3.2 - Production Ready with Anti-Crash & Data Persistence",
    version: "3.2.0",
    status: "online",
...
const server = app.listen(PORT, () => {
  console.log('╔═══════════════════════════════════════════════════╗');
  console.log('║   AuthAPI v3.2 - Production Ready Server         ║');
  console.log('╚═══════════════════════════════════════════════════╝');
  ...
  console.log('🔒 FREE USERS: API Code required for verification');
  console.log('⭐ PREMIUM USERS: API Code optional');
  console.log('💾 Auto backup: Every 6 hours');
  console.log('🛡️ Anti-crash: Enabled');
  console.log('═══════════════════════════════════════════════════');
