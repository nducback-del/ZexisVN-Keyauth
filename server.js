// server.js (Hiển thị user + pass, login ok, bcrypt-free)
...
// --- tạo config nếu không có
if (!fs.existsSync(CONFIG_FILE)) {
  const cfg = {
    admin: {
      username: ADMIN_USER,
      passwordHash: createPBKDF2Hash(ADMIN_PASS)
    }
  };
  fs.writeFileSync(CONFIG_FILE, JSON.stringify(cfg, null, 2), 'utf8');
}

// --- PBKDF2 password utils
...
// --- ADMIN INFO (hiển thị user + pass luôn)
app.get('/api/admin-info', (req,res)=>{
  const cfg = loadConfig();
  return res.json({
    username: cfg.admin.username,
    password: ADMIN_PASS // hiển thị password gốc từ env (hoặc mặc định)
  });
});
...
app.post('/api/create-key', requireAdmin, (req,res)=>{
  ...
  keys.push(record); saveKeys(keys);
  return res.json({success:true,key:record});
});

// --- Admin list/extend/reset/delete keys
...
app.post('/api/verify-key',(req,res)=>{
  ...
  const keys=loadKeys(); const found=keys.find(k=>k.key_code===key); if(!found) return res.status(404).json({success:false,message:'Key not found'});
...
app.get('/',(req,res)=>{
  ...
  // hiển thị user+pass ngay trên root
  const cfg=loadConfig();
  return res.send(`<h2>License Server Running</h2><p>Admin: ${cfg.admin.username}</p><p>Password: ${ADMIN_PASS}</p>`);
});
